...
char *SoundFont::readCompressedSample(Sample *s) { return 0; }
    ...
    for (int i = 0; i < segments; ++i) {
        ...
        for (int idx : sampleIdx) {
            ...
            fprintf(f, "Sample sample%d(%d, %d, %d, %d, %d, %d, %d, %d, wave%d);\n", idx, 0,
                    ...
                    s->loopstart, //  - s->start,
                    s->loopend,   //  - s->start,
                    s->samplerate, s->origpitch, s->pitchadj, s->sampletype, idx);
        }
        ...
        for (Zone *z : instrument->zones) {
            ...
            for (GeneratorList *g : gl) {
                if ((idx3 % 8) == 0) {
                    ...
                    fprintf(f, ", ");
                fprintf(f, "ge_%d_%d_%d(%d, %d)", idx, idx2, idx3, int(g->gen), g->amount.sword);
                ++idx3;
            }
            fprintf(f, ";\n");
            int n = gl.size();
            ...
            if (sampleIdx == -1)
                ...
                fprintf(f,
                        "static IZone iz%d_%d(%d, %d, %d, %d, &sample%d, %d, "
                        "geList_%d_%d);  // %s\n",
                ...
                fprintf(f, "static IZone iz%d_%d(%d, %d, %d, %d, &sample%d, %d, 0);  // %s\n", idx,
                        idx2, keyLo, keyHi, veloLo, veloHi, sampleIdx, n, samples[sampleIdx]->name);
            ...
            if (keyLo == 0 && keyHi == 127 && veloLo == 0 && veloHi == 127) {
                ...
                if (instrIdx == -1)
                    ...
                    fprintf(f, "static PZone pz%d_%d(%d, %d, %d, %d, &instr%d);\n", idx, idx2,
                            keyLo, keyHi, veloLo, veloHi, instrIdx);
            }
            ++idx2;
        }
...
bool SoundFont::writeCode(std::vector<int> pnums) {
    ...
    for (int idx : sampleIdx) {
        ...
        fprintf(f, "Sample sample%d(%d, %d, %d, %d, %d, %d, %d, %d, wave%d);\n", idx,
                ...
                s->loopstart - s->start, s->loopend - s->start, s->samplerate, s->origpitch,
                s->pitchadj, s->sampletype, idx);
    }
    ...
    for (int idx : pnums) {
        ...
        fprintf(f, "static Preset preset%d(%d, %d, 0, %d, pzones%d);    // %s\n\n", idx, p->preset,
                p->bank, (int)p->zones.size(), idx, p->name);
        ++idx;
    }
